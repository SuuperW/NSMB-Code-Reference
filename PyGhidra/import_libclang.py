...
def join(ghidra_script: GhidraScript):
	global script, currentProgram
	script = ghidra_script
	currentProgram = script.currentProgram
	
	global conflict_handler, symbol_table, function_manager, listing, flat_api, program_dtm
	conflict_handler = DataTypeConflictHandler.REPLACE_HANDLER
	symbol_table = currentProgram.getSymbolTable()
	function_manager = currentProgram.getFunctionManager()
	listing = currentProgram.getListing()
	flat_api = FlatProgramAPI(currentProgram)
	program_dtm = currentProgram.getDataTypeManager()
...
def _get_ghidra_type(clang_or_str: Type | str, allow_none: bool, dtm: DataTypeManager) -> DataType | None:
	if isinstance(clang_or_str, str):
		name = clang_or_str
		ghidra_type = dtm.getDataType(name)
	...
	ghidra_type = _get_ghidra_type(type_name, True, dtm)
	if ghidra_type is None:
		if clang_node.kind in pointer_types:
			ptee = clang_node.get_pointee()
			gpt = _get_ghidra_type(ptee, allow_none, dtm)
			if gpt is not None:
				ghidra_type = dtm.getPointer(gpt)
			elif allow_none:
				return None
			...
			gat = _get_ghidra_type(atype, allow_none, dtm)
			if gat is not None:
				ghidra_type = ArrayDataType(gat, asize)
			elif allow_none:
				return None
		if ghidra_type is not None:
			ghidra_type = dtm.addDataType(ghidra_type, conflict_handler)
...
def get_gram_type(clang_or_str: Type | str) -> DataType:
	ghidra_type = _get_ghidra_type(clang_or_str, True, program_dtm)
	if ghidra_type is None:
		ghidra_type = program_dtm.addDataType(get_ject_type(clang_or_str), conflict_handler)
	return ghidra_type
def get_ject_type(clang_or_str: Type | str, allow_none: bool = False) -> DataType:
	return _get_ghidra_type(clang_or_str, allow_none, project_dtm)
...
def create_empty_struct(name: str, union: bool = False):
	...
	return project_dtm.addDataType(struct, conflict_handler)
...
def create_enum(name: str, clang_node: Type):
	...
	project_dtm.addDataType(enum, conflict_handler)

def create_function_type(name: str, clang_func_type: Type, param_names = None):
	"""
	...
	func_type.setReturnType(get_ject_type(clang_func_type.get_result()))
	func_type.setCallingConvention(CompilerSpec.CALLING_CONVENTION_default)
	
	return project_dtm.addDataType(func_type, conflict_handler)

def create_function_member_pointer(name: str, clang_func_type: Type):
	"""
	...
	func_type = create_function_type(f'{class_name}::{name}_func', clang_func_type)
	...
	func_ptr = program_dtm.getPointer(func_type)
	#project_dtm.addDataType(func_ptr, conflict_handler) #?1
	struct.add(func_ptr, 0, 'func', None)
	struct.add(get_ject_type('/stddef.h/ptrdiff_t'), 0, 'offset', None)
...
def create_typedef(tdef_info: TypedefInfo) -> bool:
	...
	if tdef_info.old_name == 'code*':
		try:
			...
			_func = program_dtm.getPointer(_func)
	...
	dataType = _func or get_ject_type(tdef_info.renamed_type, True)
	if dataType is None:
	...
	project_dtm.addDataType(typedef, conflict_handler)
	return True
...
def populate_struct(class_info: ClassInfo):
	...
	ghidra_type = get_ject_type('/' + class_info.name.replace('::', '/'))
	...
	for subclass in class_info.subclasses:
		if subclass.name in types_to_ignore:
			continue
	...
	for subclass in class_info.subclasses:
		if subclass.name in types_to_ignore:
			continue
		for sub_field in get_ject_type(subclass.clang_type).getComponents():
	...
	if len(vtable_methods) > 0:
		...
		vtable_ptr_type = project_dtm.getPointer(vtable_type)
		#vtable_ptr_type = project_dtm.addDataType(vtable_ptr_type, conflict_handler)
		...
		for method in vtable_methods:
			func_type = get_ject_type('/' + method.name.replace('::', '/'), True)
			if func_type is None:
				func_type = create_function_type(method.name, method.clang_type, method.param_names)
			func_ptr_type = project_dtm.getPointer(func_type)
			#func_ptr_type = project_dtm.addDataType(func_ptr_type, conflict_handler)
			if '::~' in method.name:
	...
	for field in clang_type.get_fields():
		...
		if field_name in class_info.aligned_fields:
			...
			while (ghidra_type.getLength() & 3) != 0 or ghidra_type.getDefinedComponentAtOrAfterOffset(ghidra_type.getLength() - 1) is None:
				ghidra_type.add(get_ject_type('/u8'), 0, f'padding_{padding_count}', None)
				padding_count += 1
		...
		if ftype.kind == TypeKind.POINTER and unelaborate(ftype.get_pointee()).kind == TypeKind.FUNCTIONPROTO:
			...
			field_ghidra_type = get_ject_type(tname)
			assert field_ghidra_type is not None, tname
			field_ghidra_type = program_dtm.getPointer(field_ghidra_type)
		else:
			field_ghidra_type = get_ject_type(ftype)
...
def _create_listing_function(fi: FunctionInfo, address: Address) -> Function:
	...
	if fi.is_thiscall:
		get_gram_type('/' + parent_namespace.getName(True).replace('::', '/')) # Ghidra requires this type exist first, or it will mess up the type now and then AGAIN when parameters are set.
	...
	func.setReturnType(get_gram_type(fi.clang_type.get_result()), SourceType.USER_DEFINED)
	...
	if fi.is_thiscall:
		...
		owning_type = get_gram_type('/' + owning_type_name)
		reg_num += basic_add_param('this', program_dtm.getPointer(owning_type))
	for param in fi.clang_type.argument_types():
		param_type = get_gram_type(param)
		...
		if reg_num < 4 and registers_used + reg_num > 4:
			...
			while vars_made < registers_used:
				reg_num += basic_add_param(f'{fi.param_names[i]}_{vars_made}', get_gram_type('/u32'))
				vars_made += 1
		else:
...
def make_var_vi(vi: VarInfo):
	if vi.is_constexpr:
		return
	make_var(vi.mangled_name, vi.name, vi.class_member, get_gram_type(vi.clang_type))
...
def make_vtable_symbol(ci: ClassInfo):
	...
	ghidra_vtable = get_gram_type(f'/{owner_name}/vtable')
...
def add_builtin_types():
	...
	cat = CategoryPath('/')
	project_dtm.addDataType(bdtm.getDataType('/uchar'), conflict_handler)
	project_dtm.addDataType(bdtm.getDataType('/schar'), conflict_handler)
	project_dtm.addDataType(bdtm.getDataType('/char'), conflict_handler)
	project_dtm.addDataType(bdtm.getDataType('/ushort'), conflict_handler)
	project_dtm.addDataType(bdtm.getDataType('/short'), conflict_handler)
	project_dtm.addDataType(bdtm.getDataType('/uint'), conflict_handler)
	project_dtm.addDataType(bdtm.getDataType('/int'), conflict_handler)
	project_dtm.addDataType(bdtm.getDataType('/ulong'), conflict_handler)
	project_dtm.addDataType(bdtm.getDataType('/long'), conflict_handler)
	project_dtm.addDataType(bdtm.getDataType('/ulonglong'), conflict_handler)
	project_dtm.addDataType(bdtm.getDataType('/longlong'), conflict_handler)
	project_dtm.addDataType(bdtm.getDataType('/bool'), conflict_handler)
	project_dtm.addDataType(bdtm.getDataType('/void'), conflict_handler)
	
	# doy
	global clib_tm
	dtm_service = script.state.getTool().getService(DataTypeManagerService)
	clib_tm = None
	...
	project_dtm.addDataType(clib_tm.getDataType('/stddef.h/ptrdiff_t'), conflict_handler)
	
def run(parse_results: ParseResults, symbols: dict[str, str], ignore: list[str], dtm: DataTypeManager):
	global symbols_arm9, types_to_ignore
	symbols_arm9 = symbols
	...
	for name in parse_results.function_member_pointers:
		create_function_member_pointer(name, parse_results.function_member_pointers[name])
	...
	for dt in program_dtm.getAllDataTypes():
...
if __name__ == "__main__":
	...
	symbols = parse_symbols_file.parse_symbols(this, f'{project_root}/symbols9.x')
	
	tid = currentProgram.startTransaction('', None)
	finished = False
	try:
		main(parse_results, symbols, ['BitFlag<u32>', 'BitFlag<u8>', 'StrongBitFlag<u32>'])
		finished = True
	finally:
